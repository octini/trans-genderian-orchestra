...
The `beta` dist-tag currently points to `2.0.0-beta.3`. npm still points `latest` at the original `2.0.0-beta.0` because it was the first published version. Prefer `@beta` in public examples, automation, and validation until a future non-prerelease publish moves `latest`.
