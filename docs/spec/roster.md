...
| Horowitz | `opencode-go/qwen3.8-flash` (xhigh) | `opencode-go/muse-spark-1.2-contributor` (xhigh) | `opencode-go/kimi-k3` (max) |
...
**Rationale:**
- **Balanced = performance-tiered, quota-aware** (max on persistent judgment seats, xhigh on workhorses): Bernstein/Nirvana → `glm-5.3-flash` (max, best Go-fit agentic); Horowitz → `qwen3.8-flash` (xhigh, durable 2× Usage $30 and the strongest practical coder — cheaper cache-read/write + lower output rate than Luna cut the review seat's quota burn ~3×); Dylan/Nas/band-members → `muse-spark-1.2-contributor` (xhigh, 226,600/mo cap). Every seat fits its Go request cap. Verified in `~/.cache/opencode/models.json`: `glm-5.3-flash` supports `max`; Qwen3.8 Flash and Muse Spark top out at `xhigh` (no `max`).
