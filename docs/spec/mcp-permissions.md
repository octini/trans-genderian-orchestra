...
|---|---|---|---|---|
| **Bernstein** | direct `edit`/`grep`/`glob`/`list` tools; bash except allowlist | `read`/`websearch`/`skill`; **bash = read-only verification allowlist** (`git diff`/`status`/`log`/`show`/`rev-parse`, toolchain probes, test/lint/typecheck, read-only shell glue for compound commands) | `task` → Horowitz/Nas/Dylan/Nirvana | Bernstein-owned `bd` lifecycle runs through six primary-gated host tools (`tgo_beads_*`, verify-first, default-on with explicit-`false` kill switch); Board reads do not authorize lifecycle actions; bd init --directory unsupported, bd -C fails, must use .cwd(directory); automated recovery stays manual. direct file-search/listing tools remain denied; allowed bash commands are matched per segment |
