...
- **Dependencies** — beads (`bd` CLI), AFT, magic-context, context7. Magic-context lands on both surfaces (`ctx_*` server tools plus the TUI sidebar, with its historian always following the active preset's Dylan model + variant — reconciled on every startup and install, shape-preserving, `magicContext.historianSync: "off"` to disable); context7 registers as a hosted remote MCP (`mcp.context7 = { type: "remote", url: "https://mcp.context7.com/mcp" }`) because its own `npx ctx7 setup --opencode` is an interactive TUI that can't run non-interactively. Both AFT (`@cortexkit/aft-opencode`) and magic-context (`@cortexkit/opencode-magic-context`) self-update by default — keep their `plugin` entries at `@latest` and leave `auto_update` on (magic-context `auto_update !== false`, AFT `auto_update ?? true`). TGO auto-fills `auto_update:true` into `~/.config/cortexkit/magic-context.jsonc` when absent (never clobbers an explicit `false`) and auto-fixes exact version pins of its two deps (`@cortexkit/opencode-magic-context@0.38.0` / `@cortexkit/aft-opencode@1.2.3` → `@latest`) on install / plugin registration; partial pins like `@0.38` and ranges like `@^0.38.0` are intentionally untouched. magic-context.jsonc files containing JSONC comments/trailing commas are skipped entirely (safe-by-design) and therefore receive no auto_update fill until rewritten as plain JSON.
