...
1. **Background Job Board injection** — `experimental.chat.messages.transform` + a `chat.message` gate. The board is a read-only renderer over Beads plus a thin live-state shim, using `bd list`, `bd ready`, `bd blocked`, and `bd memories` when host setup supports those reads. Board reads do not authorize lifecycle actions — only the host-observable primary lineage plus a live lookup gates dispatch. Bernstein operates the lifecycle through six primary-gated host tools (`tgo_beads_*`, verify-first, default-on with explicit-`false` kill switch); automated recovery stays manual. bd init --directory is unsupported; bd -C fails, must use .cwd(directory).
...
Every delegation carries a **Five-part Spec**: Objective / Files / Interfaces / Constraints / Verification (plus optional `style` for voice cards: `default` | `prose` | `conversational`; precedence explicit request > packet > default, old `register` ignored). Verification is the boolean exit gate — tests pass, lint clean — that Bernstein uses when deciding whether to close the issue, via `tgo_beads_close` once the gate passes. Specialists reply with a **structured report**: STATUS (complete / partial / blocked / escalate) · CHANGES · VERIFIED · GAPS. The plugin validates delegation and reports, and the host verifies the claim live before any dispatch.
...
Beads is the work-unit store, and Bernstein is its single writer through six primary-gated host tools (create before delegating, verify-every-claim at dispatch, close only on verified completion; automated recovery stays manual). Specialists have zero beads surface. Each issue doubles as a **living spec** — explicit success criteria, bidirectional updates (implementation writes back what was built), a spec-review checkpoint before coding starts, and a decision log.
