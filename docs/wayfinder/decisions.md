...
**Q1 — Shape: Hybrid.** Config-first roster + a thin plugin core. Routing/delegation stays prompt-driven; the plugin validates lifecycle metadata and observes state, without performing Beads lifecycle writes or reimplementing control flow.
...
**Q6 — Code boundary = 4 core hooks plus an opt-in observer:** (1) Background Job Board injection (`messages.transform`/`chat.message`, sentinel-tagged, strip-and-replace for cache safety); (2) session reconciliation (`session.status`/idle events); (3) task-fit rejection normalization (`tool.execute.after`); (4) always-on concision transform (`system.transform`); and an opt-in `experimental.text.complete` observer for surrogate-only style reinforcement. Watchdog activity hooks and load-time config/setup paths are supporting lifecycle code, not core orchestra hooks. The completion observer remains inert on the verified production path without explicit preservation context and response lineage.

Apart from the documented observer and supporting lifecycle boundaries above, everything else stays out of orchestration code: routing in prompts, roster in config, models in presets.
...
**Q1 — Coupling: scoped deep integration (user agreed with recommendation).** The intended architecture uses Beads to track the *work units* the loop cares about — each delegated task maps to an issue, status machine-readable. Live issue mapping and Bernstein reconciliation via `bd list`/`bd show` are planned follow-up behavior, not current plugin-host capability. The **Background Job Board becomes a RENDERER over beads + a thin live-state shim**, NOT a parallel structure (one store, no drift). The board's context-injection function survives (the LLM must see a snapshot each turn) but is *derived from beads* in the future integration. Thin runtime layer kept only for genuinely-live state; the current plugin validates metadata only.

**Q2 — Single-writer model (user's correction; future architecture).** Bernstein is the ONLY intended beads operator:
...
- Would assign (= the claim — verified: beads has no auth layer, assignment IS the claim).
- Would mark `in_progress` at dispatch, close on verified completion, reopen on kick-back, and record Horowitz review verdicts. These lifecycle operations are planned and unsupported in the current plugin host.
...
**AMENDS tgo-a6r.17 (permission matrix):** Bernstein remains the intended single writer in the future integration; the current plugin host does not expose live `bd` lifecycle writes.

**Where the thin board lives:** the future TGO opencode-beads replacement would own context injection (beads-derived snapshot each turn) + `bd` calls; the current plugin validates metadata only. The `bd` CLI remains the engine dependency, auto-installed by the installer if missing.
...
- **Per-repo setup auto-trigger (tgo-96f.14, RESOLVED):** `SetupController` (src/setup.ts) runs on `session.created` only when the host explicitly reports `info.parentID === null`; missing `parentID` is not primary and setup is skipped. For an explicitly primary session it runs `bd init` → `bd setup opencode` → TGO AGENTS fragment, idempotent + no-clobber (marker = `.beads/` + both AGENTS markers). `setup: { enabled, autoInstallBeads }` config switches (autoInstallBeads reuses deps.ts's beads install command). The architecture s6 non-load-bearing setup skill ships as `assets/skills/tgo-setup/SKILL.md`, copied into the config dir at install. G10's D-half (merge content = TGO thin fragment + official `bd setup opencode` block) confirmed built. The 2026-08-07 live run verified the explicit-null host case in a disposable repo; setup behavior remains host-dependent.
