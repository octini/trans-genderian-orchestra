...
- **tgo-96f.9 (concision):** `experimental.chat.system.transform` (primary-loop gate via `session.parentID`). `chat.params` NOT adopted — register dial is prompt-level, no param injection.
...
- **tgo-96f.14 (setup auto-trigger, LIVE-VERIFIED 2026-08-07):** `session.created` event properties = `{ info: Session }` (sdk types.gen `EventSessionCreated`); `Session` carries `{ id, projectID, directory, parentID? }` — the plugin gates primary-only via `parentID`, reads the repo path from `directory`, and runs per-repo setup (bd init → `bd setup opencode` → TGO AGENTS fragment) idempotently. BUILT in tgo-96f.14: `SetupController` (src/setup.ts) + the non-load-bearing `tgo-setup` skill asset. **Live-verified under a real headless opencode run (2026-08-07):** `session.created` fires for the primary with `parentID:null`; SetupController ran `bd init → bd setup opencode → AGENTS fragment` in a fresh repo (created `.beads/` + a `profile:full` beads block + the TGO advice fragment). Second run in the same repo: no setup re-run (idempotent).
- **tgo-v6g (G5, LIVE-VERIFIED 2026-08-07):** headless `opencode run --format json` + `TGO_DEBUG_EVENTS=1` plugin logging against a temp config dir (installer output) in a fresh repo:
  - **`session.created` fires for subagents too** — each subagent session emits `session.created` with `parentID` = the delegating session id (never null). The setup auto-trigger's primary-only gate (`if (info.parentID) return`) therefore correctly skips per-repo setup for every subagent spawn.
