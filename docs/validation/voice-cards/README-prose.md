...
The marquee said TGO, and under that, in smaller letters, a thin orchestration plugin for OpenCode. TGO takes a single model call and turns it into a small, disciplined ensemble. You talk to Bernstein and Bernstein hands the work to the right specialist and brings the results back and checks them against the goal you actually set. It does not add magic. It keeps good work from slipping through the cracks, like a stagehand who never takes a bow and still knows where every prop lives.

If you have watched a long agent session drift, you have seen the room go soft at the edges. The agent loses the plot and rewrites the same fix and marks itself done before the tests pass, and the log scrolls past like cars on a wet road at midnight. TGO is the band that keeps tempo so the soloists can play. The soloists play. The tempo holds.

## Why this shape

They had us in a room that smelled like fresh installs and old demos, forty teams watching a harness burn tokens the way a cheap heater burns oil. The research behind TGO is honest about where agent teams wobble, and the answers are deliberately boring:
...
You do not need to take any of this on faith and the trail is in `docs/research/architectural-review.md`, and you also do not need to believe a big token-savings headline. The style-quality benchmark in `docs/spec/style-quality-evaluation.md` is explicit about what it measures: proxy token counts (words plus a little punctuation math), proxy latency and cost, and deterministic surrogate edits, not billed provider usage. It reports cost per successful task as the real tradeoff, and it labels any vendor token-reduction claim as "external, not TGO" so it cannot be mistaken for a measurement. We would rather be clear than impressive. The benchmark sits on the table like a parking meter fed until four o'clock, ticking honestly while the room decides.

## Quick start — from empty folder to working repo without a slash command

The bell over the door rang and nobody comes in happy at the exact moment you need a new repo. TGO does not ask you to run a command. It asks you to open a folder and speak.
...
Bernstein sketches the work as a dependency-ordered DAG and sends each piece out with a five-part spec (Objective / Files / Interfaces / Constraints / Verification) and waits for a structured report back (STATUS / CHANGES / VERIFIED / GAPS). The verification line is the promise you can hold him to — tests pass and lint is clean or the work is not closed. Specialists stay in their lane because the host makes them: Dylan is the only seat that can touch files and Nas never gets `bash` or `task` at all and Horowitz can read but not write and Nirvana is tool-less by design.

The Background Job Board is the view that follows you through all of this. Each turn the plugin re-derives a short board snapshot from Beads — `bd list` and `bd ready` and `bd blocked` and `bd memories` when the host exposes them — and shares it with the model as context. It is read-only context, not a license to write. Creating, claiming, closing, reopening, or recovering beads outside that read path stays disabled until the host boundary for lifecycle writes is proven; the current plugin stays metadata-only there (`beadsLifecycle.allowed: false`).
...
The board refreshes and the lanes hold. The report comes back on one line.
...
The release notes sit in a tight block on the wall. The previous governance is still audible underneath, like a previous tenant's paint showing through the new coat.
...
The roster is five names and one band. The names stay on the door.
...
The dial turns and the prose widens. The house stays where it was.

## What the engine behind it actually needs

TGO keeps its dependency surface small and explicit, like a counter with four jars and nothing behind it. The installer checks for each of these and installs what is missing (`--deps auto | check | skip`):
...
Four jars on the counter. The rest is shelf space.

## Permissions that actually enforce

Not guidelines — the seat frontmatter carries a permission matrix the host enforces, and the host does not negotiate.
...
The lanes are painted and the host holds the brush. The seats stay inside them.
...
Thirteen skills and the plugin does not lean on any of them. They wait on the shelf.
...
The style is a countertop wiped clean. The tools sit where they should.

## Deepwork when you ask for it

Autonomy is a mode, not the default. "Keep going" is opt-in and bounded — max phases and a token budget and a checkpoint rhythm and stagnation detection and three re-planning levels. When something irreversible, expensive, or direction-changing comes up, TGO pauses and asks.

You say go and it measures the hallway. It stops at the door.
...
The long form sits in docs like files on a shelf. You take the one you need.
...
You clear the cache and restart. The slot is no longer stuck.
...
He salted the next batch anyway.
