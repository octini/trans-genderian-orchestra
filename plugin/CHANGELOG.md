...
- **Variant validation fail-closed against the selectable-variant table:** per-lens overrides validate model + variant at load (`SELECTABLE_VARIANTS` — Spark ceiling xhigh, `deepseek-v4.1-flash` high/max, `qwen3.8-flash` high; `max` broken upstream: anomalyco/opencode#45987). Unknown model IDs on lens keys throw (core-seat model drift still passes through, per the locked future/model-x contract); unselectable variants throw on any seat whose model is in the selectable-variants table.
