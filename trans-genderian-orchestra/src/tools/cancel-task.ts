...
async function deleteAndVerifySession(
  ...
  // SDK shape: client.session doesn't expose .delete in @opencode-ai/sdk v2,
  // but OpenCode's runtime client provides it for session cleanup fallback.
...
function canDeleteSession(client: PluginInput['client']): boolean {
  // SDK shape: client.session doesn't expose .delete in @opencode-ai/sdk v2;
  // this capability probe keeps the runtime-only cleanup path guarded.
...
async function getSessionStatus(
  ...
  try {
    // SDK shape: client.session.status is not typed as the zero-argument
    // status-map endpoint that OpenCode exposes at runtime.
    const result = await (
...
async function getSessionParentID(
  ...
  // SDK shape: client.session doesn't expose .get in @opencode-ai/sdk v2,
  // but the runtime client returns session metadata including parentID.
