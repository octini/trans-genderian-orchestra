...
export function getCacheDir(): string {
  if (process.platform === 'win32') {
    ...
    return join(base, 'trans-genderian-orchestra', 'bin');
  }
...
export async function downloadAstGrep(
  ...
  if (!platformInfo) {
    console.error(
      `[trans-genderian-orchestra] Unsupported platform for ast-grep: ${platformKey}`,
    );
    return null;
  }
  ...
  console.log(`[trans-genderian-orchestra] Downloading ast-grep binary...`);
  ...
  try {
    ...
    console.log(`[trans-genderian-orchestra] ast-grep binary ready.`);

    return binaryPath;
  } catch (err) {
    console.error(
      `[trans-genderian-orchestra] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`,
    );
    return null;
  }
}
