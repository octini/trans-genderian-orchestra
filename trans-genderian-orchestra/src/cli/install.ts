...
const GITHUB_REPO = 'octini/trans-genderian-orchestra';
const GITHUB_URL = `https://github.com/${GITHUB_REPO}`;

function printHeader(isUpdate: boolean): void {
  console.log();
  console.log(
    `${BOLD}trans-genderian-orchestra ${isUpdate ? 'Update' : 'Install'}${RESET}`,
  );
...
async function runInstall(config: InstallConfig): Promise<number> {
  ...
  printStep(step++, totalSteps, 'Adding trans-genderian-orchestra plugin...');
  if (config.dryRun) {
