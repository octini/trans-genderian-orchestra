...
import {
  ...
  createDelegationEnforcementHook,
  createFilterAvailableSkillsHook,
  createJsonErrorRecoveryHook,
  createPathGatingHook,
  createPhaseReminderHook,
  createPostFileToolNudgeHook,
  createReadBudgetHook,
  createReviewerEnforcementHook,
...
function mergeAgentConfig(
  ...
  return deepMerge(pluginAgent, existing)!;
}
...
async function runHook(
  ...
  hookLog: typeof import('./utils/logger').log,
  phase: 'pre-tool' | 'post-tool' = 'pre-tool',
): Promise<void> {
  try {
    await fn();
  } catch (error) {
    hookLog(`[plugin] ${phase} hook failed open`, {
      hook: name,
      tool: input.tool,
...
const GLOBAL_SESSION_AGENT_MAP = new Map<string, string>();
const GLOBAL_REVIEWER_ENFORCEMENT_RESETS = new Map<
  string,
  (reason?: string) => void
>();

const TransGenderianOrchestra: Plugin = async (ctx) => {
  ...
  let readBudgetHook: ReturnType<typeof createReadBudgetHook>;
  let chatHeadersHook: ReturnType<typeof createChatHeadersHook>;
  let delegationEnforcementHook: ReturnType<
    typeof createDelegationEnforcementHook
  >;
  let reviewerEnforcementHook: ReturnType<typeof createReviewerEnforcementHook>;
  let contextOrchestratorHook: ReturnType<typeof createContextOrchestratorHook>;
  let pathGatingHook: ReturnType<typeof createPathGatingHook>;
  ...
  let pingAllCommand: ReturnType<typeof createPingAllCommand>;
  let setupSkillsHook: ReturnType<typeof createSetupSkillsHook>;
  let startupInitHook: ReturnType<typeof createStartupInitHook>;
  ...
  try {
    ...
    // Enforce delegation envelopes on orchestrator task delegations
    delegationEnforcementHook = createDelegationEnforcementHook(
      GLOBAL_SESSION_AGENT_MAP,
      { enforcementMode: config.envelopeEnforcement },
    );

    // Safety net for the mandatory reviewer gate. Prompt guidance is primary;
    // this hook logs/injects reminders when the orchestrator skips review.
    GLOBAL_REVIEWER_ENFORCEMENT_RESETS.get(ctx.directory)?.(
      'plugin reinitialization',
    );
    reviewerEnforcementHook = createReviewerEnforcementHook({
      shouldTrack: (sessionID) =>
        GLOBAL_SESSION_AGENT_MAP.get(sessionID) === 'orchestrator',
    });
    GLOBAL_REVIEWER_ENFORCEMENT_RESETS.set(
      ctx.directory,
      reviewerEnforcementHook.reset,
    );

    // Maintain Dispatcher context files for orchestration lifecycle metadata
    contextOrchestratorHook = createContextOrchestratorHook(
      ctx.directory,
      GLOBAL_SESSION_AGENT_MAP,
    );

    // Enforce role-specific write/edit path restrictions
    pathGatingHook = createPathGatingHook(
      GLOBAL_SESSION_AGENT_MAP,
      config.agentGating,
    );
    ...
    // Enforce orchestrator pure-dispatcher read/research budget
    readBudgetHook = createReadBudgetHook({
      shouldCheck: (sessionID) =>
        GLOBAL_SESSION_AGENT_MAP.get(sessionID) === 'orchestrator',
    });
    ...
    pingAllCommand = createPingAllCommand(ctx, agentDefs);
    setupSkillsHook = createSetupSkillsHook(ctx.directory);
    startupInitHook = createStartupInitHook(ctx.directory, {
      contextOrchestrator: contextOrchestratorHook,
      backgroundJobBoard,
    });
    taskSessionManagerHook = createTaskSessionManagerHook(ctx, {
      ...
      readContextMinLines: config.backgroundJobs?.readContextMinLines ?? 10,
      readContextMaxFiles: config.backgroundJobs?.readContextMaxFiles ?? 8,
      timeouts: config.timeouts ?? DEFAULT_SPECIALIST_TIMEOUTS,
      backgroundJobBoard,
      shouldManageSession: (sessionID) =>
        GLOBAL_SESSION_AGENT_MAP.get(sessionID) === 'orchestrator',
    });
    if (typeof reviewerEnforcementHook.reset === 'function') {
      reviewerEnforcementHook.reset('plugin initialization');
    }
    ...
    await appLog(
      ctx,
      'error',
      `INIT FAILED: ${String(err)}. Report at https://github.com/octini/trans-genderian-orchestra/issues`,
    );
    throw err;
  }
  ...
  if (
    ...
    const msg = [
      ...
      '  https://github.com/octini/trans-genderian-orchestra/issues',
    ].join('\n');
    log(`[plugin] WARN: ${msg}`);
  ...
  return {
    ...
    config: async (opencodeConfig: Record<string, unknown>) => {
      ...
      // Merge Agent configs — per-agent merge to preserve user-supplied fields
      // from opencode.json, while recursively merging nested objects so partial
      // user overrides do not replace all plugin defaults.
      if (!opencodeConfig.agent) {
        ...
        for (const [name, pluginAgent] of Object.entries(agents)) {
          ...
          if (existing) {
            // Plugin defaults first, user overrides win; nested objects such as
            // permission.skill and options are recursively merged.
            (opencodeConfig.agent as Record<string, unknown>)[name] =
              mergeAgentConfig(pluginAgent, existing);
          } else {
            (opencodeConfig.agent as Record<string, unknown>)[name] = {
              ...pluginAgent,
            };
      ...
      if (runtimePresetName && config.presets?.[runtimePresetName]) {
        ...
        for (const [agentName, override] of Object.entries(runtimePreset)) {
          // Resolve alias keys (e.g. "research" → "researcher")
        ...
        if (prevPresetName && config.presets?.[prevPresetName]) {
          ...
          // (handles alias keys like "research" → "researcher")
      ...
      pingAllCommand.registerCommand(opencodeConfig);
      setupSkillsHook.registerCommand(opencodeConfig);
      startupInitHook.registerCommand(opencodeConfig);
    ...
    event: async (input) => {
      ...
      if (event.type === 'session.created') {
        ...
        if (childSessionId && agent) {
          // GLOBAL_SESSION_AGENT_MAP must be populated before multiplexer
          // onSessionCreated so the multiplexer can resolve agent display names.
          GLOBAL_SESSION_AGENT_MAP.set(
            childSessionId,
            resolveRuntimeAgentName(config, agent),
          );
        }
      }
      ...
      if (input.event.type === 'session.deleted') {
        ...
        if (sessionID) {
          GLOBAL_SESSION_AGENT_MAP.delete(sessionID);
        }
      }
    },

    // Path-gating runs first and short-circuits (is_denied = true) on denial.
    // Subsequent hooks self-filter by tool name (applyPatch only on apply_patch,
    // taskSessionManager/delegationEnforcement/reviewerEnforcement/readBudget only on task).
    'tool.execute.before': async (input, output) => {
      const toolInput = input as ToolExecuteInput & { directory?: string };
      const toolOutput = output as ToolExecuteOutput;

      await pathGatingHook['tool.execute.before'](toolInput, toolOutput);

      if (toolOutput.is_denied) return;

      await runHook('applyPatch', () =>
        applyPatchHook['tool.execute.before'](toolInput, toolOutput),
        toolInput,
        log,
      );

      await runHook('taskSessionManager', () =>
        taskSessionManagerHook['tool.execute.before'](toolInput, toolOutput),
        toolInput,
        log,
      );

      await runHook('delegationEnforcement', () =>
        delegationEnforcementHook['tool.execute.before'](toolInput, toolOutput),
        toolInput,
        log,
      );

      await runHook('reviewerEnforcement', () =>
        reviewerEnforcementHook['tool.execute.before'](toolInput, toolOutput),
        toolInput,
        log,
      );

      await runHook('readBudget', () =>
        readBudgetHook['tool.execute.before'](toolInput, toolOutput),
        toolInput,
        log,
      );
    },
    ...
    'command.execute.before': async (input, output) => {
      const commandInput = input as CommandExecuteInput;
      ...
      await pingAllCommand.handleCommandExecuteBefore(
        commandInput,
        commandOutput,
      );

      await setupSkillsHook.handleCommandExecuteBefore(
        commandInput,
        commandOutput,
      );

      await startupInitHook.handleCommandExecuteBefore(
        commandInput,
        commandOutput,
      );
    },
    ...
    'chat.message': async (
      ...
      if (agent) {
        GLOBAL_SESSION_AGENT_MAP.set(input.sessionID, agent);
      }
    },
    ...
    'experimental.chat.system.transform': async (
      ...
      const agentName = input.sessionID
        ? GLOBAL_SESSION_AGENT_MAP.get(input.sessionID)
        : undefined;
      if (agentName === 'orchestrator') {
        const alreadyInjected = output.system.some(
          (s) =>
            typeof s === 'string' && s.includes(ORCHESTRATOR_PROMPT_SENTINEL),
        );
        if (!alreadyInjected) {
    ...
    'experimental.chat.messages.transform': async (
      ...
      // Strip image parts from orchestrator messages when @researcher is
      ...
      await reviewerEnforcementHook['experimental.chat.messages.transform'](
        input,
        typedOutput,
      );
    ...
    'tool.execute.after': async (input, output) => {
      const toolInput = input as ToolExecuteInput;
      const toolOutput = output as ToolExecuteOutput;

      await runHook('delegate-task-retry', () =>
        delegateTaskRetryHook['tool.execute.after'](toolInput, toolOutput),
        toolInput,
        log,
        'post-tool',
      );

      await runHook('json-error-recovery', () =>
        jsonErrorRecoveryHook['tool.execute.after'](toolInput, toolOutput),
        toolInput,
        log,
        'post-tool',
      );

      await runHook('post-file-tool-nudge', () =>
        postFileToolNudgeHook['tool.execute.after'](toolInput, toolOutput),
        toolInput,
        log,
        'post-tool',
      );

      await runHook('task-session-manager', () =>
        taskSessionManagerHook['tool.execute.after'](toolInput, toolOutput),
        toolInput,
        log,
        'post-tool',
      );

      await runHook('reviewer-enforcement', () =>
        reviewerEnforcementHook['tool.execute.after'](toolInput, toolOutput),
        toolInput,
        log,
        'post-tool',
      );

      await runHook('context-orchestrator', () =>
        contextOrchestratorHook['tool.execute.after'](
          toolInput,
...
export default TransGenderianOrchestra;
