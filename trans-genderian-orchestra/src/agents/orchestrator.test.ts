...
describe('buildOrchestratorPrompt', () => {
  ...
  test('requires reviewer gate and rejection escalation loop', () => {
    ...
    expect(prompt).toContain(
      'Every non-trivial work delegation (to @builder, @researcher, @planner) MUST be followed by a delegation to @reviewer.',
    );
    expect(prompt).toContain(
      'Trivial tasks (typo fixes, simple lookups) may skip the reviewer gate only when explicitly marked with `"trivial": true` in the delegation envelope',
    );
    expect(prompt).toContain(
  ...
  test('ends with the orchestrator prompt sentinel', () => {
    const prompt = buildOrchestratorPrompt();
