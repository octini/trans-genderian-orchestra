...
describe('startup init hook command registration', () => {
  ...
  test('registers /beads:init command when absent', () => {
    ...
    expect(command.description).toContain('@beads/bd init');
  });
...
describe('startup init hook command handling', () => {
  ...
  test('seeds AGENTS.md from bundled template when no template override is provided', async () => {
    await withTempDir(async (dir) => {
      const hook = createStartupInitHook(dir, {
        execCommand: async (cmd) => {
          if (cmd[0] === 'git' && cmd[1] === 'rev-parse') {
      ...
      await hook.handleCommandExecuteBefore(
        { command: 'init', sessionID: 's1', arguments: '' },
        output,
      );

      const agentsContent = await readFile(path.join(dir, 'AGENTS.md'), 'utf8');
      expect(agentsContent).toContain(`# ${path.basename(dir)} Agent Guide`);
      expect(agentsContent).toContain('### Orchestrator');
      expect(agentsContent).toContain('### Builder');
      expect(output.parts[0].text).toContain('Created AGENTS.md');
    });
  });

  test('seeds AGENTS.md once when /init commands run concurrently', async () => {
    await withTempDir(async (dir) => {
      const templatePath = path.join(dir, 'template.md');
      await writeFile(templatePath, '# {{project_name}}\n', 'utf8');
      const hook = createStartupInitHook(dir, {
      ...
      const firstOutput = { parts: [{ type: 'text', text: 'template' }] };
      const secondOutput = { parts: [{ type: 'text', text: 'template' }] };
      ...
      await Promise.all([
        ...
        hook.handleCommandExecuteBefore(
          { command: 'init', sessionID: 's2', arguments: '' },
          secondOutput,
        ),
      ]);
      ...
      expect(agentsContent).toBe(`# ${path.basename(dir)}\n`);
      expect(
        [firstText, secondText].some((text) =>
          text.includes('Created AGENTS.md'),
        ),
      ).toBe(true);
      expect(
        [firstText, secondText].some((text) =>
          text.includes('AGENTS.md already exists; left unchanged.'),
        ),
      ).toBe(true);
    });
  });
  ...
  test('handles /init:all command with skills fallback after primary verification fails', async () => {
    await withTempDir(async (dir) => {
      const templatePath = path.join(dir, 'template.md');
      await writeFile(templatePath, '# {{project_name}}\n', 'utf8');
      const calls: string[][] = [];
      ...
      const hook = createStartupInitHook(dir, {
        ...
        packageVerifier: async (packageName) => {
          verifierCalls.push(packageName);
          return packageName !== '@opencode-ai/skills-installer';
        },
      });
      const output = { parts: [{ type: 'text', text: 'template' }] };
      ...
      expect(output.parts[0].text).toContain(
        'Package verification failed for @opencode-ai/skills-installer',
      );
      expect(output.parts[0].text).toContain(
        'npx --yes setup-matt-pocock-skills',
      );
  ...
  test('skips verified npx commands when package verification fails', async () => {
    await withTempDir(async (dir) => {
      ...
      const hook = createStartupInitHook(dir, {
        ...
        packageVerifier: async (packageName) => packageName !== '@beads/bd',
      });
      const output = { parts: [{ type: 'text', text: 'template' }] };
...
describe('startup init command executor', () => {
  test('executes commands without relying on Bun.spawn', async () => {
    const originalDisableBunSpawn = process.env.DISPATCHER_DISABLE_BUN_SPAWN;

    try {
      process.env.DISPATCHER_DISABLE_BUN_SPAWN = '1';
      ...
      expect(result).toEqual({ success: true, stdout: 'node ok', stderr: '' });
    } finally {
      if (originalDisableBunSpawn === undefined) {
        delete process.env.DISPATCHER_DISABLE_BUN_SPAWN;
      } else {
        process.env.DISPATCHER_DISABLE_BUN_SPAWN = originalDisableBunSpawn;
      }
    }
  });
});
