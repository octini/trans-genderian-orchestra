...
describe('setup-matt-pocock-skills command hook', () => {
  ...
  test('expands /setup-matt-pocock-skills into setup guidance', async () => {
    ...
    expect(output.parts[0].text).toContain(
      'Orchestrator: dispatching-parallel-agents, brainstorming, grill-with-docs, trans-genderian-orchestra',
    );
  ...
  test('tries fallback installer when primary package verification fails', async () => {
    const calls: string[][] = [];
    ...
    const hook = createSetupSkillsHook(
      ...
      async (packageName) => {
        ...
        return packageName === 'setup-matt-pocock-skills';
      },
    );
    const output = { parts: [{ type: 'text', text: 'template' }] };
    ...
    expect(calls).toEqual([['npx', '--yes', 'setup-matt-pocock-skills']]);
    expect(output.parts[0].text).toContain(
