...
describe('post-file-tool-nudge hook', () => {
  ...
  test('appends delegation reminder to research tool outputs', async () => {
    const hook = createPostFileToolNudgeHook();
    ...
    for (const tool of [
      ...
      const output = createOutput(`${tool} content`);

      await hook['tool.execute.after']({ tool, sessionID: 's1' }, output);
