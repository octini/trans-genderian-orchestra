...
import type { ToolExecuteInput } from '../types';
...
function extractTaskSummary(
  output: unknown,
): Pick<DelegationLogEntry, 'summary' | 'touchedFiles'> {
  if (typeof output !== 'string') {
    return {
      summary: 'Task completed (no text output)',
      touchedFiles: [],
    };
  }
  ...
  if (summaryMatch) {
    try {
      ...
      const whatChanged =
        typeof summary.what_changed === 'string'
          ? summary.what_changed.trim()
          : '';
      if (whatChanged) {
        return {
          summary: whatChanged,
          touchedFiles: Array.isArray(summary.files_touched)
            ? summary.files_touched.filter(
      ...
      // Fall back to legacy raw-output summary below.
    }
  }
...
export function createContextOrchestratorHook(
  ...
  return {
    'tool.execute.after': async (
      ...
      const taskSummary = extractTaskSummary(output?.output);

      await updateStateMd({
        taskId: input.callID ?? 'unknown',
        agent: 'specialist',
        status: 'completed',
        touchedFiles: taskSummary.touchedFiles,
