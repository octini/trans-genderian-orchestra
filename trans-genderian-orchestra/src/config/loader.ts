...
export function mergePluginConfigs(
  ...
  return {
    ...
    agentGating: deepMerge(base.agentGating, override.agentGating),
    tmux: deepMerge(base.tmux, override.tmux),
    multiplexer: deepMerge(base.multiplexer, override.multiplexer),
    interview: deepMerge(base.interview, override.interview),
    backgroundJobs: deepMerge(base.backgroundJobs, override.backgroundJobs),
    timeouts: mergeSpecialistTimeouts(base.timeouts, override.timeouts),
    fallback: deepMerge(base.fallback, override.fallback),
    council: deepMerge(base.council, override.council),
  };
}

function mergeSpecialistTimeouts(
  base?: SpecialistTimeoutsConfig,
  override?: SpecialistTimeoutsConfig,
): SpecialistTimeoutsConfig {
  if (!base) return override ?? DEFAULT_SPECIALIST_TIMEOUTS;
  if (!override) return base;

  return {
    planner:
      override.planner === DEFAULT_SPECIALIST_TIMEOUTS.planner
        ? base.planner
        : override.planner,
    researcher:
      override.researcher === DEFAULT_SPECIALIST_TIMEOUTS.researcher
        ? base.researcher
        : override.researcher,
    builder:
      override.builder === DEFAULT_SPECIALIST_TIMEOUTS.builder
        ? base.builder
        : override.builder,
    reviewer:
      override.reviewer === DEFAULT_SPECIALIST_TIMEOUTS.reviewer
        ? base.reviewer
        : override.reviewer,
    council:
      override.council === DEFAULT_SPECIALIST_TIMEOUTS.council
        ? base.council
        : override.council,
    councillor:
      override.councillor === DEFAULT_SPECIALIST_TIMEOUTS.councillor
        ? base.councillor
        : override.councillor,
  };
}
...
export function loadPluginConfig(
  ...
  let config: PluginConfig = userConfigPath
    ? (loadConfigFromPath(userConfigPath, options) ?? ({} as PluginConfig))
    : ({} as PluginConfig);
  ...
  const envPreset = process.env.TRANS_GENDERIAN_ORCHESTRA_PRESET;
  if (envPreset) {
