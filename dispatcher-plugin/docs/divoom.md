...
trans-genderian-orchestra can mirror agent activity to a Divoom MiniToo Bluetooth
...
"/Applications/Divoom MiniToo.app/Contents/Resources/.venv/bin/python" \
  ...
  --max-frames 24 \
  --posterize-bits 3 \
  --out-dir ~/.local/share/opencode/storage/trans-genderian-orchestra/divoom/captures
```
...
`$XDG_DATA_HOME/opencode/storage/trans-genderian-orchestra/divoom/captures` when
`XDG_DATA_HOME` is set to a non-empty absolute path. Otherwise it falls back to
`~/.local/share/opencode/storage/trans-genderian-orchestra/divoom/captures`.

## Enable in trans-genderian-orchestra
...
```bash
TRANS_GENDERIAN_ORCHESTRA_DIVOOM=1 opencode
```
