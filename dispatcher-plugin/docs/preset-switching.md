...
1. Define named presets in `trans-genderian-orchestra.jsonc` under the `presets` field
...
| Method | How | Persists? |
|--------|-----|-----------|
| Config file | Set `"preset": "cheap"` in `trans-genderian-orchestra.jsonc` | Yes, across restarts |
