...
Built-in Model Context Protocol (MCP) servers ship with trans-genderian-orchestra and give agents access to external tools — web search, library documentation, and code search.
...
Control which MCPs each agent can use via the `mcps` array in your preset config (`~/.config/opencode/trans-genderian-orchestra.json` or `.jsonc`):
