...
function mergeAgentConfig(
  ...
  if (
    isPlainRecord(pluginAgent.permission) &&
    isPlainRecord(existing.permission)
  ) {
    merged.permission = {
...
const TransGenderianOrchestra: Plugin = async (ctx) => {
  ...
  return {
    ...
    config: async (opencodeConfig: Record<string, unknown>) => {
      ...
      // from opencode.json, while deep-merging permission so a partial user
      // permission override does not replace all plugin defaults.
      if (!opencodeConfig.agent) {
        ...
        for (const [name, pluginAgent] of Object.entries(agents)) {
          ...
          if (existing) {
            // Plugin defaults first, user overrides win; permission is merged
            // one level deeper so user permissions are additive/overriding.
    ...
    'tool.execute.before': async (input, output) => {
      const runBeforeHook = async (
        ...
        try {
          ...
          // Log but don't throw — let subsequent hooks run.
          const meta = input as {
            tool?: string;
            sessionID?: string;
            callID?: string;
          };
          log('[plugin] pre-tool hook failed open', {
            hook: name,
            tool: meta.tool,
      ...
      await runBeforeHook('applyPatch', () =>
        applyPatchHook['tool.execute.before'](
          input as {
            tool: string;
            directory?: string;
          },
      ...
      await runBeforeHook('taskSessionManager', () =>
      ...
      await runBeforeHook('delegationEnforcement', () =>
        delegationEnforcementHook['tool.execute.before'](
          input as {
            tool: string;
            sessionID?: string;
            callID?: string;
      ...
      await runBeforeHook('reviewerEnforcement', () =>
        reviewerEnforcementHook['tool.execute.before'](
          input as {
            tool: string;
            sessionID?: string;
            callID?: string;
          },
          output as { args?: unknown; output?: unknown },
        ),
      );

      await runBeforeHook('readBudget', () =>
        readBudgetHook['tool.execute.before'](
          input as {
            tool: string;
            sessionID?: string;
            callID?: string;
      ...
      await runBeforeHook('pathGating', () =>
        pathGatingHook['tool.execute.before'](
          input as {
            tool: string;
            sessionID?: string;
            callID?: string;
    ...
    'tool.execute.after': async (input, output) => {
      ...
      await runPostToolHook('reviewer-enforcement', () =>
        reviewerEnforcementHook['tool.execute.after'](
          input as {
            tool: string;
            sessionID?: string;
            callID?: string;
      ...
      await runPostToolHook('context-orchestrator', () =>
