...
const READ_ONLY_TOOL_PERMISSIONS: PermissionMap = {
  '*': 'deny',
  read: 'allow',
  glob: 'allow',
  grep: 'allow',
...
const READ_BASH_TOOL_PERMISSIONS: PermissionMap = {
  ...READ_ONLY_TOOL_PERMISSIONS,
  bash: 'allow',
};
...
const READ_BASH_WEB_TOOL_PERMISSIONS: PermissionMap = {
  ...READ_BASH_TOOL_PERMISSIONS,
  webfetch: 'allow',
  websearch: 'allow',
};

const PLANNER_TOOL_PERMISSIONS: PermissionMap = {
  ...READ_BASH_WEB_TOOL_PERMISSIONS,
  write: 'allow',
  edit: 'allow',
};

const RESEARCHER_TOOL_PERMISSIONS: PermissionMap = {
  ...READ_BASH_WEB_TOOL_PERMISSIONS,
  write: 'allow',
  edit: 'allow',
};

const AGENT_TOOL_PERMISSIONS: Record<string, PermissionMap> = {
  orchestrator: ORCHESTRATOR_TOOL_PERMISSIONS,
  planner: PLANNER_TOOL_PERMISSIONS,
  researcher: RESEARCHER_TOOL_PERMISSIONS,
  builder: {
    '*': 'deny',
    question: 'allow',
    read: 'allow',
  ...
  reviewer: READ_ONLY_TOOL_PERMISSIONS,
  council: READ_ONLY_TOOL_PERMISSIONS,
  councillor: {
    ...READ_ONLY_TOOL_PERMISSIONS,
    question: 'deny',
  },
...
function mergeAgentOverrides(
  base: AgentOverrideConfig | undefined,
  override: AgentOverrideConfig | undefined,
...
function getEffectiveAgentOverride(
  config: PluginConfig | undefined,
  name: string,
): AgentOverrideConfig | undefined {
  return mergeAgentOverrides(
    DEFAULT_AGENT_OVERRIDES[name as keyof typeof DEFAULT_AGENT_OVERRIDES],
    getAgentOverride(config, name),
  );
}
 ...
 * Sets role-based tool permissions, 'question' permission to 'allow', and
 * includes skill permission presets.
...
function applyDefaultPermissions(
  ...
  const rolePermissions =
    AGENT_TOOL_PERMISSIONS[agent.name] ?? READ_BASH_TOOL_PERMISSIONS;
  const existing =
    typeof agent.config.permission === 'object' && agent.config.permission
      ? (agent.config.permission as PermissionMap)
      : {};
...
export function createAgents(config?: PluginConfig): AgentDefinition[] {
  ...
  const protoCustomAgents = customAgentNames.flatMap((name) => {
    const override = getEffectiveAgentOverride(config, name);
    if (!hasCustomAgentModel(override)) {
      console.warn(
        `[trans-genderian-orchestra] Custom agent '${name}' skipped: 'model' is required`,
      );
      return [];
    }
  ...
  // 2. Apply default permissions, then overrides to built-in subagents
  const builtInSubAgents = protoSubAgents.map((agent) => {
    const override = getEffectiveAgentOverride(config, agent.name);
  ...
  if (legacyMasterModel) {
    ...
    if (
      councilAgent &&
      !getEffectiveAgentOverride(config, 'council')?.model &&
  ...
  const orchestratorOverride = getEffectiveAgentOverride(
    config,
    'orchestrator',
  );
...
export function getDisabledAgents(config?: PluginConfig): Set<string> {
  ...
  if (!config?.council || disabled.has('council')) {
    // Council is opt-in; councillor is internal-only and is gated with it
    // so it cannot be registered or spawned when council support is disabled.
