...
describe('orchestrator read budget hook', () => {
  ...
  test('increments read count on read/glob calls', async () => {
    ...
    expect(third.output).toContain(
      'You have made 3 consecutive read/research calls',
    );
  ...
  test('injects warning at threshold 3', async () => {
    const hook = createReadBudgetHook();
    ...
    expect(output.output).toContain('third read');
    expect(output.output).toContain(
      '[SYSTEM: You have made 3 consecutive read/research calls.',
    );
