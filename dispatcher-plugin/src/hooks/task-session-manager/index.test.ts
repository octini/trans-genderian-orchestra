...
function createHook(options?: {
  ...
  timeouts?: Parameters<typeof createTaskSessionManagerHook>[1]['timeouts'];
}) {
...
describe('task-session-manager hook', () => {
  test('stores background task launches in job board prompt context', async () => {
    ...
    expect(userMessage.parts[0].text).toContain(
      'res-1 / child-1 / researcher / running',
    );
    expect(userMessage.parts[0].text).toContain(
  ...
  test('applies configured timeout to managed task_status wait calls', async () => {
    const board = new BackgroundJobBoard();
    const { hook } = createHook({
      backgroundJobBoard: board,
      timeouts: { builder: 1500000 },
    });
    ...
    await hook['tool.execute.before'](
      { tool: 'task', sessionID: 'parent-1', callID: 'call-1' },
      {
        args: {
          ...
          description: 'implement timeout plumbing',
        },
      },
    );
    await hook['tool.execute.after'](
  ...
  test('does not overwrite explicit task_status timeout_ms', async () => {
    const board = new BackgroundJobBoard();
    ...
    const { hook } = createHook({
      backgroundJobBoard: board,
      timeouts: { researcher: 600000 },
    });

    const output = {
      args: { task_id: 'res-1', wait: true, timeout_ms: 1234 },
    };
    await hook['tool.execute.before'](
      { tool: 'task_status', sessionID: 'parent-1', callID: 'call-1' },
      output,
    );
  ...
  test('updates background job board from task_status output', async () => {
    ...
    expect(messages.messages[0].parts[0].text).toContain(
      'rev-1 / child-1 / reviewer / completed, unreconciled',
    );
    expect(messages.messages[0].parts[0].text).toContain(
  ...
  test('keeps task_status timeout as a running timed-out job', async () => {
    ...
    expect(messages.messages[0].parts[0].text).toContain(
      'bld-1 / child-1 / builder / running, timed out',
    );
  });
  ...
  test('updates background job board from injected completion messages', async () => {
    ...
    expect(messages.messages[0].parts[0].text).toContain(
      'res-1 / child-1 / researcher / completed, unreconciled',
    );
  });
  ...
  test('completed reconciled job appears reusable and resumes via task', async () => {
    ...
    expect(nextMessages.messages[0].parts[0].text).toContain(
      'res-1 / child-1 / researcher / completed, reconciled',
    );
  ...
  test('only reconciled completed jobs resolve as reusable task sessions', async () => {
    ...
    const unreconciled = {
      args: { subagent_type: 'reviewer', task_id: 'rev-1' },
    };
    await hook['tool.execute.before'](
      { tool: 'task', sessionID: 'parent-1', callID: 'call-1' },
      unreconciled,
    ...
    const failed = { args: { subagent_type: 'reviewer', task_id: 'rev-2' } };
    await hook['tool.execute.before'](
    ...
    const completed = { args: { subagent_type: 'reviewer', task_id: 'rev-1' } };
    await hook['tool.execute.before'](
    ...
    await hook['experimental.chat.messages.transform']({}, messages);
    expect(messages.messages[0].parts[0].text).toContain(
      'rev-1 / done-1 / reviewer / completed, reconciled',
    );
  ...
  test('running alias is polled through task_status but not resumed by task', async () => {
    ...
    const resume = { args: { subagent_type: 'researcher', task_id: 'res-1' } };
    await hook['tool.execute.before'](
  ...
  test('task alias is dropped when subagent_type is missing', async () => {
    ...
    const resume = { args: { task_id: 'res-1' } };
    await hook['tool.execute.before'](
      { tool: 'task', sessionID: 'parent-1', callID: 'resume' },
      resume,
    );
  ...
  test('task alias is dropped when subagent_type is invalid', async () => {
    ...
    const resume = {
      args: { subagent_type: 'not-an-agent', task_id: 'res-1' },
    };
    await hook['tool.execute.before'](
      { tool: 'task', sessionID: 'parent-1', callID: 'resume' },
      resume,
  ...
  test('wrong parent or wrong agent alias does not resolve', async () => {
    ...
    const wrongAgent = {
      args: { subagent_type: 'reviewer', task_id: 'res-1' },
    };
    await hook['tool.execute.before'](
    ...
    const wrongParent = { args: { task_id: 'res-1' } };
    await hook['tool.execute.before'](
      { tool: 'task_status', sessionID: 'parent-2', callID: 'parent' },
      wrongParent,
    );
    expect(wrongParent.args.task_id).toBe('res-1');
  });
  ...
  test('bare task id output without state does not create reusable job', async () => {
    ...
    await hook['tool.execute.before'](
      { tool: 'task', sessionID: 'parent-1', callID: 'call-1' },
      { args: { subagent_type: 'researcher', description: 'legacy output' } },
    );
    await hook['tool.execute.after'](
      { tool: 'task', sessionID: 'parent-1', callID: 'call-1' },
  ...
  test('reads before and after launch attach with unique-line counts and caps', async () => {
    ...
    await hook['tool.execute.before'](
      { tool: 'task', sessionID: 'parent-1', callID: 'call-1' },
      { args: { subagent_type: 'researcher', description: 'context caps' } },
    );
    await hook['tool.execute.after'](
      { tool: 'task', sessionID: 'parent-1', callID: 'call-1' },
      { output: ['task_id: child-1', 'state: running'].join('\n') },
  ...
  test('parent deletion clears jobs and pending calls', async () => {
    ...
    await hook['tool.execute.before'](
      { tool: 'task', sessionID: 'parent-1', callID: 'call-1' },
      {
        args: { subagent_type: 'reviewer', description: 'architecture review' },
      },
    );
    board.registerLaunch({
      taskID: 'child-1',
