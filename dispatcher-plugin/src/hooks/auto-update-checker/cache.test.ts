  ...
  describe('resolveInstallContext', () => {
    test('detects OpenCode packages install root from runtime package path', async () => {
      const existsSpy = spyOn(fs, 'existsSync').mockImplementation(
        (p: string) =>
          p ===
          '/home/user/.cache/opencode/packages/trans-genderian-orchestra@latest/package.json',
      );
      ...
      const context = resolveInstallContext(
        '/home/user/.cache/opencode/packages/trans-genderian-orchestra@latest/node_modules/trans-genderian-orchestra/package.json',
      );

      expect(context).toEqual({
        installDir:
          '/home/user/.cache/opencode/packages/trans-genderian-orchestra@latest',
        packageJsonPath:
          '/home/user/.cache/opencode/packages/trans-genderian-orchestra@latest/package.json',
      });
  ...
  describe('preparePackageUpdate', () => {
    ...
    test('updates packages wrapper dependency and removes installed package', async () => {
      const existsSpy = spyOn(fs, 'existsSync').mockImplementation(
        (p: string) =>
          p ===
            '/home/user/.cache/opencode/packages/trans-genderian-orchestra@latest/package.json' ||
          p ===
            '/home/user/.cache/opencode/packages/trans-genderian-orchestra@latest/node_modules/trans-genderian-orchestra',
      );
        ...
        (p: string) => {
          if (
            ...
            return JSON.stringify({
              dependencies: {
                'trans-genderian-orchestra': '0.9.1',
              },
            });
          }
          return '';
      ...
      expect(JSON.parse(writtenData[0])).toEqual({
        dependencies: {
          'trans-genderian-orchestra': '0.9.11',
        },
      });
    ...
    test('keeps working when dependency is already on target version', async () => {
      const existsSpy = spyOn(fs, 'existsSync').mockImplementation(
        (p: string) =>
          ...
          p.endsWith('/.cache/opencode/node_modules/trans-genderian-orchestra'),
      );
      const readSpy = spyOn(fs, 'readFileSync').mockReturnValue(
        JSON.stringify({
          dependencies: {
            'trans-genderian-orchestra': '1.0.1',
          },
        }),
      );
      const writeSpy = spyOn(fs, 'writeFileSync').mockImplementation(() => {});
