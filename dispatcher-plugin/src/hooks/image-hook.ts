...
export function processImageAttachments(args: {
  ...
  const researcherEnabled = !disabledAgents.has('researcher');
  ...
  for (const { msg, imageParts } of messagesWithImages) {
    ...
    msg.parts = msg.parts
      ...
      .concat([
        {
          type: 'text',
          text: `[Image attachment detected.${pathsText} Your model may not support image input. Delegate to @researcher with the file path(s) above so it can read the file with its read tool.]`,
        },
      ]);
  }
}
