...
TGO (**trans-genderian-orchestra**) is a custom multi-agent orchestration plugin for [OpenCode](https://opencode.ai). It coordinates a fixed roster of named, role-anchored subagents (a "band") around one primary orchestrator agent. TGO is configuration-first: routing and delegation are prompt-driven; a thin plugin core verifies claims and observes state, never reimplementing control flow — Beads lifecycle writes run only through Bernstein's six primary-gated host tools.
...
7. `docs/spec/concision-enforcement.md` — the always-on 3-axis house style (now card-aware; see `docs/spec/voice-cards.md` for voice cards + rule packs).
...
- **Voice cards** — selectable prose voices; `tgo-default` always-on for every seat, named cards (`tgo-prose`, `tgo-conversational`) assigned via delegation packet, explicit request (`use prose` / `use conversational`), or ask-when-ambiguous; precedence explicit > packet > default; off-switch `stop X` / `normal mode`; old `register` is ignored, config is `style.card`.
- **Rule packs** — loadable drift families: `mechanics` always-on, `concision` whitelist-gated, `voice-cadence` cluster-judged; plus findings-targeted nudges with flag-then-override (`rhythm`/`emphasis`/`picture`/`idiom`/`joke`).
- **Scrub list** — the always-on banned-tells list (filler, hedges, AI-vocab, etc.), now inside rule packs.
...
TGO is a hub-and-spoke orchestra with a single writer. **Bernstein**, the primary, turns each goal into a dependency-ordered DAG of work units, each a living-spec beads issue with a boolean exit gate. He dispatches same-level tasks together as waves to the specialist seats — **Dylan** (sole writer), **Nas** (read-only lookup), **Horowitz** (review/advisor) — and convenes **Nirvana** (three tool-less lenses + synthesizer) for judgment-heavy calls. Bernstein is its single writer (create before delegating, claim at dispatch, close only on verified completion per docs/spec/beads-integration.md); the plugin host verifies every claim and performs Bernstein-owned create, claim, close, dep, update, and reopen through six primary-gated tools (default-on, explicit-`false` kill switch; automated recovery stays manual) — see that spec for the host boundary (bd init --directory unsupported, bd -C fails, must use .cwd(directory)). All seats speak in one uniform, always-on house style (`tgo-default`) enforced by a per-turn system transform and folded seat prompts; it carries a plain-language spine, banned tells judged by clusters, STE thresholds (20/25), and preservation-first, with drift gated by the three rule packs. Named voices (`tgo-prose`, `tgo-conversational`) layer on default when assigned via packet, explicit request, or ask-when-ambiguous; Dylan self-classify is fallback for unassigned creative tasks only. Enforcement is by capabilities, not compliance: the Orchestrator literally cannot edit, grep, or glob; specialists can only do their lane. A thin plugin core provides four core runtime hooks (job-board injection, session reconciliation, task-fit rejection normalization, concision transform) plus an opt-in completion observer for surrogate-only style reinforcement; everything else lives in config, prompts, and presets. Autonomy is a mode, not the default — deepwork is opt-in with hard bounds, and a checkpoint protocol pauses on irreversible, expensive, or direction-changing actions.
...
- Balanced preset: split routing — Bernstein/Horowitz/Nirvana-synth → `glm-5.3-flash` (max), Nas/Dylan/band-members → Muse Spark (xhigh). Cheap preset: Muse Spark (xhigh) on every seat. Frontier preset: Bernstein → `opencode-go/glm-5.3` (max), Horowitz → `opencode-go/kimi-k3` (max), Nirvana → `opencode-go/grok-4.6` (xhigh), Dylan/Nas/band-members → Muse Spark (xhigh). Muse Spark and Grok 4.6 cap at `xhigh`; `glm-5.3`, `kimi-k3`, and `glm-5.3-flash` support `max`.
