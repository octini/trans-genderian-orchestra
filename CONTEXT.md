...
- Balanced preset: Bernstein/Nirvana → `opencode-go/glm-5.3-flash` (max), Horowitz → `opencode-go/qwen3.8-flash` (xhigh), Dylan/Nas/band-members → `opencode-go/muse-spark-1.2-contributor` (xhigh). Cheap preset: Muse Spark (xhigh) on every seat. Frontier preset: Bernstein → `opencode-go/glm-5.3` (max), Horowitz → `opencode-go/kimi-k3` (max), Nirvana → `opencode-go/grok-4.6` (xhigh), Dylan/Nas/band-members → Muse Spark (xhigh). Muse Spark, Grok 4.6, and Qwen3.8 Flash cap at `xhigh`; the rest support `max`.
