...
describe('custom-agent creation', () => {
  ...
  test('supports prompt and conductorPrompt for custom agents', () => {
    const config: PluginConfig = {
      agents: {
        'test-auditor': {
    ...
    const conductor = agents.find((agent) => agent.name === 'conductor');
    expect(conductor?.config.prompt).toContain(
      '@test-auditor\n- Role: Compliance audit specialist',
    );
  });
  ...
  test('skips custom agents without a model', () => {
    ...
    try {
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        "[TGO] Custom agent 'janitor' skipped: 'model' is required",
      );
    } finally {
      warnSpy.mockRestore();
    }
  ...
  test('does not create or inject disabled custom agents', () => {
    const config: PluginConfig = {
      disabled_agents: ['test-auditor', 'composer'],
      agents: {
        'test-auditor': {
          model: 'openai/gpt-5.4-mini',
  ...
  test('accepts arbitrary conductorPrompt text for custom agents', () => {
    const config: PluginConfig = {
      agents: {
        janitor: {
          model: 'openai/gpt-5.4-mini',
          conductorPrompt: '@cleanup\n- Role: Cleanup specialist',
        },
      },
    };
