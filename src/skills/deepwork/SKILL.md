---
name: deepwork
description: Conductor-only workflow for heavy coding sessions, multi-phase implementation, and risky refactors. Use for complex work that needs planning, review gates, and persistent progress tracking.
---

# Deepwork

Deepwork is a Conductor workflow for heavy coding sessions. Use it when the
...
When deepwork is active, the Conductor must manage the work as a scheduler,
...
- ask `@principal` to review the plan and revise it until acceptable;
- create a phased implementation/delegation plan;
- before Principal reviews, add relevant confirmed research findings and file
  references to the deepwork file so Principal can review the plan or phase from
  accepted context instead of redoing discovery;
- ask `@principal` to review that implementation plan before execution;
- after Principal review and before each implementation phase, decide the execution
...
- after each phase, validate, update the deepwork file, prepare the plan file
  for Principal review and ask `@principal` to review the phase result, fix
  actionable issues, then continue;
- when a phase includes UI/UX work by `@composer`, preserve the accepted design
  intent across later phases and route follow-up UI/UX changes back to
  `@composer` with explicit scope;
- finish with final validation and a concise summary.

## Composer UI/UX Handoff Guardrail

When a deepwork phase includes UI/UX work by `@composer`, treat the delivered UI/UX as
accepted design intent for later phases. Record any important design decisions in
the deepwork file before continuing.

After UI/UX Composer work:
...
- route follow-up visual, responsive, motion, hierarchy, polish, or
  component-feel changes back to `@composer` with the design intent included;
- keep mechanical follow-up scoped so it preserves the accepted design exactly,
  such as wiring, tests, type fixes, or non-visual behavior changes;
...
- researched, factual context from `@scribe` to avoid Principal doing its own
  research;
- plan drafts and Principal review notes;
...
When `@scribe` docs, code reads, or external references produce useful
...
- follow Conductor delegation rules;
...
- avoid blocking the Conductor lane while background jobs run; if no independent
