...
description: Configure and improve trans-genderian-orchestra for the current user. Use when users want to tune agents, models, prompts, custom agents, skills, MCPs, presets, or plugin behavior. Also use when recurring workflow friction suggests a safe config or prompt improvement.
---

# trans-genderian-orchestra Configuration Skill

You help users configure, customize, and safely improve their
trans-genderian-orchestra setup.
...
trans-genderian-orchestra is configured through a plugin config file, usually:
...
| `~/.config/opencode/trans-genderian-orchestra.jsonc` | User plugin config with comments/trailing commas; takes precedence over `.json` |
| `~/.config/opencode/trans-genderian-orchestra.json` | User plugin config generated by installer |
| `<project>/.opencode/trans-genderian-orchestra.json` | Project-local plugin overrides checked before user config |
| `~/.config/opencode/trans-genderian-orchestra/{agent}.md` | Full prompt replacement for a built-in agent |
| `~/.config/opencode/trans-genderian-orchestra/{agent}_append.md` | Append-only prompt tuning for a built-in agent |
...
1. If a `preset` is active, check
   `~/.config/opencode/trans-genderian-orchestra/{preset}/` first.
...
- **Limit costs**: use cheaper models for `scribe`, `librarian`, and `fixer`.
- **Improve quality**: use stronger models for `conductor`, `principal`, or
...
- **Guide delegation**: add `orchestratorPrompt` for custom agents so the
  Conductor knows when to call them and when not to.

Important schema boundary:

- Built-in agents (`conductor`, `principal`, `librarian`, `scribe`,
...
- Built-in agent `prompt` and `orchestratorPrompt` fields are **not** supported
  in `trans-genderian-orchestra.json[c]`; use markdown prompt override files instead.
...
```text
~/.config/opencode/trans-genderian-orchestra/openai/conductor_append.md
```
...
1. **Inspect current setup**
   - Read the existing trans-genderian-orchestra config file.
...
- codebase scouting → `scribe`
- external docs/research → `librarian`
- architecture/debugging/review → `principal`
...
- The Conductor repeatedly delegates too much or too little for this user's
  workflow.
...
```text
I can add a narrow Conductor prompt rule to avoid parallel writer agents in
the same directory unless their file ownership is explicit. Target:
~/.config/opencode/trans-genderian-orchestra/conductor_append.md. Apply it?
```
...
public API changes to it. Apply to your trans-genderian-orchestra config?
```
