...
It is Conductor-owned. The Conductor delegates source discovery and URL/ref
resolution to `@scribe`, asks for approval, then performs the git and
filesystem operations directly.

There is intentionally no helper script. Dependency discovery, ref validation,
and cloning are repo-specific enough that the Conductor/Scribe workflow is
...
metadata. After cloning, the Conductor should add or update a concise
...
before asking `@scribe` for new recommendations.
