...
function activationPrompt(task: string): string {
  return [
    ...
    '- draft a plan and get `@principal` review before implementation;',
    '- create and review a phased implementation/delegation plan;',
    '- execute phase by phase with background specialists where useful;',
    '- wait for hook-driven background completion, reconcile results, validate, and ask `@principal` to review each phase;',
    '- ask `@principal` to include simplify/readability feedback in phase reviews;',
