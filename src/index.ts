...
const OhMyOpenCodeLite: Plugin = async (ctx) => {
  ...
  return {
    ...
    'experimental.chat.messages.transform': async (
      ...
      // Strip image parts from conductor messages. When the conductor's
      // model doesn't support image input, the API call fails before the
      // LLM can respond. We replace image bytes with a text nudge so the
      // conductor can use the saved file path(s) with a read tool.
