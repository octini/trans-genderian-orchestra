...
export function createTgoCommandConfigs(): Record<string, TgoCommandConfig> {
  return {
    'tgo:doctor': {
      ...
      template:
        'Run the deterministic TGO doctor workflow by executing npx --yes trans-genderian-orchestra@beta doctor --json from the current project directory. Do not run bd doctor; Beads diagnostics are separate from TGO doctor output.',
    },
    'tgo:setup': {
      description:
