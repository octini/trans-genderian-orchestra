...
function extractReadFiles(
  root: string,
  output: ToolExecuteOutput & { metadata?: unknown },
): ContextFile[] {
...
export function createTaskSessionManagerHook(
  ...
  options: {
    ...
    timeouts?: Partial<Record<AgentName, number>>;
  ...
  function timeoutForAgent(agent: AgentName): number | undefined {
    return options.timeouts?.[agent];
  }
  ...
  async function handleTransientTaskStatusOutput(
    output: ToolExecuteOutput & { metadata?: unknown },
  ): Promise<boolean> {
  ...
  return {
    'tool.execute.before': async (
      ...
      if (toolName === 'task_status') {
        const args = output.args as TaskStatusArgs;
        ...
        if (resolved) {
          args.task_id = resolved.taskID;
          if (args.wait === true && args.timeout_ms === undefined) {
            const timeoutMs = timeoutForAgent(resolved.agent as AgentName);
      ...
      if (typeof args.subagent_type === 'string') {
        args.subagent_type = args.subagent_type.toLowerCase();
      }
      ...
      const pendingCall: PendingTaskCall = {
        ...
        agentType: args.subagent_type,
        label,
        timeoutMs: timeoutForAgent(args.subagent_type),
      };
  ...
  function normalizeLateCancelledToolStatus(
    output: ToolExecuteOutput & { metadata?: unknown },
  ): void {
    if (typeof output.output !== 'string') return;
    const status = parseTaskStatusOutput(output.output);
