...
import { DEFAULT_SPECIALIST_TIMEOUTS, type PluginConfig } from '../config';
import {
...
export class CouncilManager {
  ...
  async runCouncil(
    ...
    const timeout =
      this.config?.timeouts?.councillor ??
      councilConfig.timeout ??
      DEFAULT_SPECIALIST_TIMEOUTS.councillor;
