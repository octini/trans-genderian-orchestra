...
Bundled skills are installed by the `trans-genderian-orchestra` installer.
...
Control which skills each agent can use in `~/.config/opencode/trans-genderian-orchestra.json` (or `.jsonc`):
